...
export function PrayerDots({ rows, size = 26 }: { rows: PrayerRow[]; size?: number }) {
  return (
    <div className="flex gap-2">
      {rows.map((r) => {
        const cls = r.logged === "on_time" ? "bg-good text-bg" : r.logged === "late" ? "bg-warn text-bg" : r.state === "open" ? "border-2 border-accent text-accent-2 pulse" : "border-2 border-line text-muted";
        return (
