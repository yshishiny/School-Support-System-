...
export default async function LessonPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  return <Stage sessionId={row.id} scriptId={s.id} character={characterById(row.character_id)} script={{ title: s.title, minutes: s.minutes, beats: s.script.beats, quiz: s.script.quiz }} language={language} startBeat={row.beat_index} minutes={s.minutes} cloudVoices={cloudVoiceList(language)} video={videoEnabled() && cloudVoiceList(language).length > 0} />;
}
