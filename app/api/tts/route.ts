...
export async function POST(req: Request) {
  ...
  try {
    const audio = await synthesize({ text: await spokenText(body.text, body.voice.startsWith("ar") ? "ar" : "en"), voice: body.voice, rate: Number(body.rate) || 1, pitch: Number(body.pitch) || 1 });
