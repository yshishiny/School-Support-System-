...
export async function loadPlan(studentId: string): Promise<PlanOverview> {
  ...
  const [{ data: timetable }, { data: topics }, { data: attempts }, { data: quizzes }, { data: covered }, { data: flags }, { data: coach }] = await Promise.all([
    ...
    admin.from("topic_flags").select("topic_id, created_at").eq("student_id", studentId).eq("kind", "not_taken"),
  ...
  const { wanted, missing } = planSlots({
    ...
    notTaken: new Set(((flags ?? []) as { topic_id: string; created_at: string }[]).filter((f) => !(covered ?? []).some((c) => c.topic_id === f.topic_id)).map((f) => f.topic_id)),
