...
export async function computeIntegrity(studentId: string, today: string, tz: string): Promise<IntegritySignal[]> {
  ...
  const [{ data: attempts }, { data: prayers }, { data: checkins }, { data: logs }, { data: snaps }, { data: materials }, { data: tt }, { data: mannersRows }, { data: tickRows }] = await Promise.all([
    ...
    admin.from("snaps").select("taken_on, status, ai_verdict, kind, ai_detail").eq("student_id", studentId).gte("taken_on", weekAgo),
  ...
  const { data: limitRow } = await admin.from("families").select("screen_limit_minutes").eq("id", (await admin.from("profiles").select("family_id").eq("id", studentId).single()).data?.family_id ?? "").maybeSingle();
  ...
  return integritySignals({
    ...
    snaps: (snaps ?? []) as { taken_on: string; status: string; ai_verdict: string | null; kind?: string; ai_detail?: { total_minutes?: number | null; top_apps?: { app: string; minutes: number }[] } | null }[],
    screenLimit: limitRow?.screen_limit_minutes ?? 180,
