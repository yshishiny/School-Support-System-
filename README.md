...
- **Get to know me** (`/me/about-me`): ten friendly questions (how they learn best, what motivates them, confidence before quizzes, how they take mistakes, preferred coach tone, dream job). Not a clinical tool: it only shapes the coach's tone, lesson style and explanation length. First completion pays +15.
- **Fluffy design**: rounded cards with soft glows, Fredoka display font, floating emoji stickers per theme, bouncy bottom nav.
- **Coach tab** (`/coach`): a confidential chat with the coach (motivational-interviewing style, never a therapist, never lectures), scheduled check-ins (weekly pulse; WHO-5 Well-Being Index monthly; mindset and body/study habits every six weeks), and the child's plan. Answers and chat stay with the child: parents see a green/amber/red light on Progress and nothing else. Every child message and free-text note is risk-classified; a high-risk turn (self-harm, abuse, drugs, being harmed) sends the parent a short Telegram alert with no quotes and shows the child Egyptian helplines; persistent low wellbeing sends a gentle amber nudge. Private one-line coach notes feed the AI only and are never surfaced to the parent report.
- **Subject icons** everywhere (🧮 Math, ⚡ Physics, ✍️ Arabic, 🕌 Religion…).
- **Tour and demo**: a first login opens `/tour` (eight slides with a three-question demo quiz that saves nothing); finishing pays +10, and it can be replayed from Me. Parents have `/parent/guide` with the weekly loop and a good-first-week checklist.
- **Clinician summary** (`/parent/clinician/<child>`): a parent-generated, printable information summary for a child and adolescent psychiatrist or psychologist, in the sections of a standard psychiatric assessment (AACAP practice parameter). It reports the WHO-5 with published cut-offs, marks the app's own questionnaires as non-validated and the AI's notes as machine-generated, lists safety flags and how they were handled, and never diagnoses. Themes from the child's private chat are included only with the child informed and agreeing, themes only, and the child sees in the app that it happened.
- **Early-warning signals** (`lib/signals.ts`): deterministic, deliberately sensitive rules over everything the app sees (weekly pulse, WHO-5 items and drops, mindset, habits, late-night use, engagement drops, broken streaks, low check-in moods, feelings written into "stuck on", repeated low-risk chats, flagged sets). A single meaningful signal reaches the "watch" tier; more load reaches amber; a moderate-risk chat plus load reaches red. Parents see the tier and plain labels on Progress and in the daily report, never the answers. Snapshots are stored nightly; the coach and the clinician summary receive them.
- **Closed loop**: the foundation topics the coach names become priority topics that the planner schedules first, and its per-subject levels set the difficulty.
- **Guidance from a professional**: on Progress the parent can paste a specialist's recommendations per child; the coach report and the private chat follow them, and the child is never told they exist. Engagement signals (missed check-ins, drops) only fire once an account is two weeks old.
- **Allowance through the basics** (`/parent/allowance`): the weekly allowance (default 250 EGP, Thursday) is earned by KPIs: dish, manners and phone judged by the parent with one tap a day on Home (only an explicit ✗ counts against him), and prayers, check-ins, planned-quiz attempts and coach check-ins counted by the app. Weighted score out of 100, graded bands (90+ full, 70+ pays 70%, 50+ pays 40%, under 50 nothing), a live meter on the child's home page, automatic close the morning after pay day with a Telegram summary and a "mark paid" history. Consequences ("practices") are a catalogue the parent enables (never schoolwork, always a way back); the child sees them on Today and can claim the earn-back for the parent to confirm. Reward ideas can be enabled with one tap on the Rewards page.
- **Hero pictures**: parents (Kids page) or the child (Me) upload pictures to a private bucket; the child picks one as avatar and one as the banner behind the Today header, and frames the banner on Me: "Full picture" (whole image over a blurred copy of itself, banner grows to fit) or "Fill & crop" (drag to move, pinch or slider to zoom from 0.5× to 3×). Served through one-hour signed URLs, visible to the family only.
- **Three home layouts**, switchable by the child on Me ("My home page") or by the parent on Kids: A three ranked things, B one thing now, C hero picture with bento tiles. All share the same ranked queue and data.
- **Today page = one thing now** (layout B, the default): a top bar (avatar, points, prayer pill), one big "Now" card chosen by a ranked queue (open prayer → evening check-in after 18:00 → today's planned quizzes → due coach check-in → recall → reviews → catch-up → intake), an "Up next" list, one "This week" bar (allowance, streak, quizzes, prayers), today's classes as chips, and the coach's line as a speech bubble. The check-in form lives on `/checkin`; the week's quizzes on Learn → For me; the full allowance meter on Rewards.
- **Entries log**: every login and the first page of each day per device are recorded with IP, city/country (from the platform's geo headers), device and browser. Parents see the list on Reports; each day's entries per account, the parent's included, are appended to the daily Telegram report.
- **Last seen (with consent)**: when a child submits a check-in or logs a prayer, his phone asks once to share location; if he allows, the position is stored with that action. Parents see "Last seen … map" on Home and a "Last location today" line in the daily report. Nothing runs in the background and the child is told on the form. Live tracking and step counts need the phone's own family tools (Find My / Family Link) or a native app.
- **Home and school places** (More → Home and school): coordinates, a Google Maps link, or an address (geocoded through OpenStreetMap). "Last seen" then reads Home, School, or "1.8 km from home", on the parent Home and in the daily report.
- **School announcements, weekly**: parents add the school website's news page under Import; every six days the nightly job re-reads it and, when the text changed, extracts dated announcements for approval (same flow as WhatsApp imports) and pings Telegram. Facebook and Instagram are refused up front because they need a login.
