...
- **Hero pictures**: parents (Kids page) or the child (Me) upload pictures to a private bucket; the child picks one as avatar and one as the banner behind the Today header, and frames the banner (drag to move, pinch or slider to zoom) on Me. Served through one-hour signed URLs, visible to the family only.
