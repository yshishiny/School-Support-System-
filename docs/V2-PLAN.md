...
| 2.3b | 8 | **Illustrated lessons — done (beta.5):** step-by-step scenes cued to the teacher's words, with a pointer and glow on the element being explained; presenter voice gender | yes |
| 2.3b+ | 8 | **Professional pictures — done (beta.6, beta.7):** a dedicated illustrator pass per scene, real photographs from Wikimedia behind the board, readability guard on labels, and exact maths for geometry and graphs (parallels, right angles, true intersections, angle arcs on their rays); a laser pointer on the step being explained; check answers rendered as presenter clips so the video teacher never freezes | yes |
| 2.3c | 9 | Interactive manipulatives (drag a point, change an angle, slide a value on a graph), adaptive re-explanation after a missed check, Quran lessons, coach and checkpoint integration, lesson line in the parent's report | yes |
